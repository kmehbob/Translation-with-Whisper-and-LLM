...
describe("DELETE /api/v1/recordings/:id (soft delete only - never touches the server copy)", () => {
    test("hides the recording (404s on re-fetch) WITHOUT deleting its row or its audio file", async () => {
        const rec = seedRecording();
        ...
        // The whole point of a soft delete: the real file is never touched.
        expect(deleteSpy).not.toHaveBeenCalled();

        const followUp = await request(app).get(`/api/v1/recordings/${rec.id}`);
        expect(followUp.status).toBe(404);

        // ...but the row is still really there, just flagged hidden.
        const stillThere = recordingsRepo.getById(rec.id);
        expect(stillThere).not.toBeNull();
        expect(stillThere.hidden).toBe(1);

        deleteSpy.mockRestore();
    });

    test("hidden recordings drop out of the list endpoint", async () => {
        const rec = seedRecording();
        await request(app).delete(`/api/v1/recordings/${rec.id}`);

        const listRes = await request(app).get("/api/v1/recordings");
        expect(listRes.body.items.some((item) => item.id === rec.id)).toBe(false);
    });

    test("hidden recordings can no longer be played, exported, or renamed", async () => {
        const rec = seedRecording();
        await request(app).delete(`/api/v1/recordings/${rec.id}`);

        const audioRes = await request(app).get(`/api/v1/recordings/${rec.id}/audio`);
        expect(audioRes.status).toBe(404);

        const exportRes = await request(app).get(`/api/v1/recordings/${rec.id}/export`).query({ format: "txt" });
        expect(exportRes.status).toBe(404);

        const renameRes = await request(app).patch(`/api/v1/recordings/${rec.id}`).send({ originalFilename: "new-name.mp3" });
    ...
    test("404s when deleting an already-hidden recording", async () => {
        const rec = seedRecording();
        await request(app).delete(`/api/v1/recordings/${rec.id}`);
        const res = await request(app).delete(`/api/v1/recordings/${rec.id}`);
        expect(res.status).toBe(404);
...
describe("PATCH /api/v1/recordings/:id (rename)", () => {
    test("renames a recording's display name", async () => {
        const rec = seedRecording();
        const res = await request(app).patch(`/api/v1/recordings/${rec.id}`).send({ originalFilename: "my-renamed-call.mp3" });
        expect(res.status).toBe(200);
        expect(res.body.originalFilename).toBe("my-renamed-call.mp3");

        const stored = recordingsRepo.getById(rec.id);
        expect(stored.original_filename).toBe("my-renamed-call.mp3");
    });

    test("404s for an unknown id", async () => {
        const res = await request(app).patch("/api/v1/recordings/does-not-exist").send({ originalFilename: "x.mp3" });
        expect(res.status).toBe(404);
    });

    test("rejects an empty name", async () => {
        const rec = seedRecording();
        const res = await request(app).patch(`/api/v1/recordings/${rec.id}`).send({ originalFilename: "   " });
        expect(res.status).toBe(400);
    });

    test("rejects a name over 255 characters", async () => {
        const rec = seedRecording();
        const res = await request(app).patch(`/api/v1/recordings/${rec.id}`).send({ originalFilename: "a".repeat(256) });
        expect(res.status).toBe(400);
    });
});

describe("POST /api/v1/recordings/restore", () => {
    test("un-hides a deleted recording within the given date range", async () => {
        const rec = seedRecording();
        await request(app).delete(`/api/v1/recordings/${rec.id}`);
        expect(recordingsRepo.getById(rec.id).hidden).toBe(1);

        const today = new Date().toISOString().slice(0, 10);
        const res = await request(app).post("/api/v1/recordings/restore").send({ dateFrom: today, dateTo: today });

        expect(res.status).toBe(200);
        expect(res.body.restored).toBeGreaterThanOrEqual(1);
        expect(recordingsRepo.getById(rec.id).hidden).toBe(0);

        const listRes = await request(app).get("/api/v1/recordings");
        expect(listRes.body.items.some((item) => item.id === rec.id)).toBe(true);
    });

    test("does not restore recordings outside the given date range", async () => {
        const rec = seedRecording();
        await request(app).delete(`/api/v1/recordings/${rec.id}`);

        const res = await request(app).post("/api/v1/recordings/restore").send({ dateFrom: "2000-01-01", dateTo: "2000-01-02" });
    ...
    test("restores everything hidden when no date range is given", async () => {
        const rec = seedRecording();
        await request(app).delete(`/api/v1/recordings/${rec.id}`);
